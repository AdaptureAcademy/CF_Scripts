...
def get_zones_by_registrar(account_id, headers):
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to retrieve registrar for domains for {account_id}: {response.status_code} - {response.text}")
        return None
...
def main():
    ...
    data = get_zones_by_registrar(account_id=account_id, headers=headers)
