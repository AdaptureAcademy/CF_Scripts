...
def get_account_lists(account_id, headers):
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to retrieve account lists for {account_id}: {response.status_code} - {response.text}")
        return None
...
def main():
    ...
    data = get_account_lists(account_id=account_id, headers=headers)
